...
router.get("/profile", isAuthenticated, async (req, res) => {
	...
	let identifikasiMember = 'Powered By Wudysoft';
	res.render('profile', {
...
router.post("/register", async (req, res, next) => {
	try {
		...
		return validasi(req, res, "API Wudysoft || VERIFICATION", next);
	} catch (err) {
		console.log(err);
...
router.post("/updates", async (req, res) => {
	...
	let bool = ["false", "true"];
	const requests = (status, result) => res.json({ status, creator: "Wudysoft", result });
	if ((email.length || limit.length || expired.length) < 1) {
		...
		let resulter;
		try {
			resulter = await getJson("https://wudysoft.up.railway.app/api/cekkey?apikey=" + anu.apikey, { method: 'GET' });
		} catch (er) {
			console.log(er);
